...
func prepareGitArgs() (logArgs, diffArgs []string) {
	...
	if err != nil {
		log.Fatalf("Could not compute merge base between ORIGIN_MASTER and %s: %s", gitCommitFrom, err.Error())
	}

	toAndMaster, err := git.MergeBase(gitRepo, "ORIGIN_MASTER", gitCommitTo)
	if err != nil {
		log.Fatalf("Could not compute merge base between ORIGIN_MASTER and %s: %s", gitCommitTo, err.Error())
	}
